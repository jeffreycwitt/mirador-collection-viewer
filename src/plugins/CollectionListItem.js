...
class CollectionListItem extends Component {
  ...
  this.state.collections.forEach((m) => {
    this.props.dispatch(actions.removeManifest(m["@id"]))
    });
  }
  handleSelectAll(){
     this.state.collections.forEach((m) => {
       this.props.dispatch(actions.fetchManifest(m["@id"]))
    });
    this.props.dispatch(actions.setWorkspaceAddVisibility(true))
  }
    ...
    if (!this.state.open){
      Axios.get(this.props.id).then((res) => {
        ...
        if (res.data.collections){
          ...
          this.props.dispatch(actions.fetchManifest(this.props.id))
        }
      })
      ...
      const displayArray = this.state.collections.map((c) => {
        if (this.state.filter){
          if (c.label.includes(this.state.filter)){
            return <CollectionListItem key={c["@id"]} label={c.label} id={c["@id"]} dispatch={this.props.dispatch} level={this.props.level + 1} filter={this.state.filter}/>
          }
        }
        else{
          return <CollectionListItem key={c["@id"]} label={c.label} id={c["@id"]} dispatch={this.props.dispatch} level={this.props.level + 1} filter={this.state.filter}/>
